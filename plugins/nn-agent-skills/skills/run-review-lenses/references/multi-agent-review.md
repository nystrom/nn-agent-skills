...
Attach each finding to the queue item it lands on so the consumer can surface it
at the right change.
...
- Its final step **posts a comment to the PR** via `gh`. That must not happen
  here — this skill returns findings and posts nothing (see Spawning).
...
- **A lens must only *return* findings — never act.** No subagent may post to
  ...
  list and nothing else. Nothing in this skill posts or edits — findings are
  recorded, and the consumer decides what to do with them. This is the reason the
  `code-review` command is applied by reading its file and skipping its final steps
  rather than run as a command: run whole, it would comment on the PR before the
  reviewer has seen anything. A
